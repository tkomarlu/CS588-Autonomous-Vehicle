...
# Description: show vehicle's heading and position in image                                                                
# Author: Hang Cui
...
image_file  = 'gnss_map_base.png'
# image_file  = 'gnss_map_demo.png'
curr_path = os.path.abspath(__file__) 
image_path = curr_path.split('scripts')[0] + 'images/' + image_file

waypoint_file = 'figure8xy.csv'
waypoint_path = curr_path.split('scripts')[0] + 'waypoints/' + waypoint_file

class GNSSImage(object):

    global image_path
    global waypoint_path
    ...
    def __init__(self):
        ...
        self.file = open(waypoint_path, 'w', newline='')
    ...
    def start_gi(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.set_initial is False:
                self.lat_start_bt = self.lat
                self.lon_start_l = self.lon
            ...
            # Output raw lat, long, and heading to waypoints csv file for gem_gnss_pp_tracker_pid.py to read in
            self.filewriter.writerow([self.lat-self.lat_start_bt, self.lon-self.lon_start_l, self.heading])
